    ...
    <div className="min-h-screen">
      ...
      <section className="bg-gradient-to-br from-blue-600 to-indigo-800 text-white py-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <SearchInput placeholder="搜索设备名称、型号..." onSearch={v => { setSearch(v); setPage(1); }} size="lg" className="max-w-xl mx-auto" />
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        {loading ? (
          ...
          <>
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {equipment.map((eq) => {
                const s = statusMap[(eq.status as string)] || statusMap.UNAVAILABLE;
                const lab = eq.lab as { nameZh: string; city?: string } | null;
                return (
                  <Link key={eq.id as string} href={`/equipment/${eq.slug as string}`}>
                    <Card hover padding="none" className="overflow-hidden">
                      ...
                      <div className="p-4">
                        <div className="flex items-start justify-between mb-2">
                          <h3 className="font-semibold text-gray-900">{eq.nameZh as string}</h3>
                          <Badge variant={s.variant}>{s.label}</Badge>
                        </div>
                        {eq.model && <p className="text-sm text-gray-500 mb-1">型号: {eq.model as string}</p>}
                        {lab && <p className="text-sm text-gray-400">{lab.nameZh}{lab.city ? ` · ${lab.city}` : ''}</p>}
                      </div>
                    </Card>
                  </Link>
                );
